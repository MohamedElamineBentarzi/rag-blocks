"""Generate `blocks.json` — the manifest that drives rag-blocks Studio.

Studio (the optional React app under `studio/app`) is a *static* site: it has no
Python at runtime. Everything it knows about the library — which blocks exist,
what each one's ports are, what params it takes, and how it's documented — comes
from this one generated file. This script is the only bridge between the two, run
at build time.

Why generate instead of hand-write: the whole library is Open/Closed via the
registry (AGENTS.md §2.4). A newly registered component must appear in the UI
with zero hand-editing, so the manifest is pure introspection over
`registry.available()` + each component's `Config` dataclass. Nothing here is
Studio-specific knowledge that could drift from the code — the code *is* the
source, read the same way `PipelineBuilder` reads it.
...
The shape it emits is documented in `studio/app/src/manifest/types.ts`.
"""
...
# The script is a dev tool, not part of the shipped package; make the repo
# importable when run directly from a source checkout.
